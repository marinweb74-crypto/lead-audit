...
# --- Font registration: Times New Roman ---
_FONT = "Helvetica"
...
def _build_prompt(lead: dict) -> str:
    ...
    data_block = textwrap.dedent(f"""\
        ...
        Запросов в месяц: {lead.get('monthly_searches', 'нет данных')}
        Конкурентов всего: {lead.get('competitors_total', 0)}
        Конкурентов с сайтом: {lead.get('competitors_with_site', 0)}
        Конкуренты в выдаче: {', '.join(competitors_list[:5]) if competitors_list else 'нет данных'}
        Упущенных клиентов/мес: {lead.get('lost_clients_low', 0)}-{lead.get('lost_clients_high', 0)}
        Потенциальная выручка/мес: {lead.get('monthly_loss', 0):.0f} руб.
...
# ---------------------------------------------------------------------------
# PDF generation — approved template with tables
# ---------------------------------------------------------------------------

def generate_pdf(lead: dict, audit_text: str, channel: str = "telegram") -> str:
    """Generate PDF audit. channel='telegram' or 'email' — affects CTA."""
    ...
    s.append(Spacer(1, 12*mm))  # push to next page area
    ...
    total = lead.get("competitors_total", 0) or 1
    with_site = lead.get("competitors_with_site", 0)
    ...
    monthly = lead.get("monthly_loss", 0)
    monthly_searches = lead.get("monthly_searches", 0)

    t3 = Table([
        ["Показатель", "Значение", "Источник"],
        ["Запросов в месяц", f"~{monthly_searches}", "Яндекс Вордстат"],
        ["Конверсия в нише", f"{conv_low}-{conv_high}%", "Среднее по рынку"],
        ["Потенц. обращений", f"{lead.get('lost_clients_low', 0)}-{lead.get('lost_clients_high', 0)}/мес", "Расчёт"],
    ...
    t3_style = TableStyle([
        ...
        ("TEXTCOLOR", (2, 1), (2, -1), GRAY),  # Sources in gray
        ("FONTSIZE", (2, 1), (2, -1), 8),
        # Last row — green bold
    ...
    # === FUNNEL — воронка клиентов ===
    s.append(Paragraph("Воронка клиентов", ss["HH"]))
    lost_low = lead.get("lost_clients_low", 0)
    lost_high = lead.get("lost_clients_high", 0)
    ...
    clients = int(monthly_searches * avg_conv * 0.7)
    ...
    scenario_rows = [
        ...
        ["Сайт", f"{clients//2}-{clients}", f"{(clients//2)*avg_check:,}-{clients*avg_check:,} ₽".replace(",", " ")],
    ...
    # For telegram — no CTA text needed, they're already in TG
...
# Keep old generate_pdf signature for backward compatibility
_generate_pdf_new = generate_pdf
...
def _make_season_chart(category: str) -> Drawing:
    """Столбчатая диаграмма сезонности."""
...
# Old generate_pdf removed — using new version above
...
def _save_audits(audits: list[dict]):
    ...
    with open(AUDITS_JSON, "w", encoding="utf-8") as f:
...
def run(config: dict):
    ...
    for lead in leads:
        ...
        try:
            ...
            logger.error("Gemini API error «%s»: %s", name, exc)
            continue
        except Exception as exc:
            logger.error("Ошибка «%s»: %s", name, exc, exc_info=True)
            continue
