"""
auditor.py — Генерация персонализированных аудитов через Gemini API + PDF.

Для каждого обогащённого лида:
1. Формирует промпт с данными компании
2. Генерирует аудит через Gemini API
3. Создаёт PDF (reportlab) — профессиональный отчёт, не рекламная листовка
4. Сохраняет результат в audits.json и помечает лида в БД
"""

import os
import json
import logging
import random
import re
import tempfile
import textwrap
import time
import requests
from datetime import datetime

from reportlab.lib.pagesizes import A4
from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
from reportlab.lib.units import mm
from reportlab.lib.enums import TA_JUSTIFY, TA_CENTER, TA_LEFT
from reportlab.platypus import (
    SimpleDocTemplate, Paragraph, Spacer, HRFlowable,
    Table, TableStyle, KeepTogether,
)
from reportlab.lib.colors import HexColor
from reportlab.pdfbase import pdfmetrics
from reportlab.pdfbase.ttfonts import TTFont
from reportlab.graphics.shapes import Drawing, Rect, String

from db import init_db, get_leads_for_audit, mark_audit_generated

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

PDF_DIR = os.path.join(PROJECT_ROOT, "agent-runtime", "outputs", "pdfs")
SHARED_DIR = os.path.join(PROJECT_ROOT, "agent-runtime", "shared")
AUDITS_JSON = os.path.join(SHARED_DIR, "audits.json")
AUDIT_LOG = os.path.join(SHARED_DIR, "audit-log.md")

NICHE_STATS = {
    "Шиномонтаж": {"avg_check": 2500, "mobile_search": 73, "conversion_low": 0.05, "conversion_high": 0.08},
    "Клининг": {"avg_check": 4000, "mobile_search": 81, "conversion_low": 0.08, "conversion_high": 0.12},
    "Автосервис": {"avg_check": 5000, "mobile_search": 76, "conversion_low": 0.06, "conversion_high": 0.09},
    "Ремонт телефонов": {"avg_check": 3500, "mobile_search": 85, "conversion_low": 0.10, "conversion_high": 0.15},
    "Массаж": {"avg_check": 3000, "mobile_search": 77, "conversion_low": 0.08, "conversion_high": 0.12},
}

logger = logging.getLogger("auditor")
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(name)s] %(message)s")

BLACK = HexColor("#222222")
GRAY = HexColor("#777777")
LGRAY = HexColor("#aaaaaa")
LINE = HexColor("#d0d5dd")
LBGR = HexColor("#f5f5f5")
RED = HexColor("#cc3333")
GREEN = HexColor("#2d8a4e")
LRED = HexColor("#fdecea")
LGREEN = HexColor("#e8f5e9")
ACCENT = HexColor("#1a5276")

_FONT = "Helvetica"
_FONT_BOLD = "Helvetica-Bold"

for _regular, _bold_path in [
    ("C:/Windows/Fonts/times.ttf", "C:/Windows/Fonts/timesbd.ttf"),
    ("/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf",
     "/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman_Bold.ttf"),
    (os.path.join(PROJECT_ROOT, "fonts", "times.ttf"), os.path.join(PROJECT_ROOT, "fonts", "timesbd.ttf")),
]:
    if os.path.exists(_regular):
        try:
            pdfmetrics.registerFont(TTFont("TNR", _regular))
            _FONT = "TNR"
            if os.path.exists(_bold_path):
                pdfmetrics.registerFont(TTFont("TNRB", _bold_path))
                _FONT_BOLD = "TNRB"
            else:
                _FONT_BOLD = "TNR"
            logger.info("Font loaded from %s", _regular)
        except Exception as exc:
            logger.warning("Font error: %s", exc)
        break


def _make_styles():
    ss = getSampleStyleSheet()
    ss.add(ParagraphStyle("TT", fontName=_FONT_BOLD, fontSize=18, leading=22, alignment=TA_CENTER, textColor=ACCENT, spaceAfter=2*mm))
    ss.add(ParagraphStyle("SB", fontName=_FONT, fontSize=11, leading=14, alignment=TA_CENTER, textColor=GRAY, spaceAfter=5*mm))
    ss.add(ParagraphStyle("HH", fontName=_FONT_BOLD, fontSize=13, leading=16, textColor=ACCENT, spaceBefore=5*mm, spaceAfter=2*mm))
    ss.add(ParagraphStyle("BB", fontName=_FONT, fontSize=11, leading=15, alignment=TA_JUSTIFY, textColor=BLACK, spaceAfter=1.5*mm))
    ss.add(ParagraphStyle("FF", fontName=_FONT, fontSize=8, leading=10, alignment=TA_CENTER, textColor=LGRAY))
    ss.add(ParagraphStyle("SM", fontName=_FONT, fontSize=9, leading=12, textColor=GRAY))
    ss.add(ParagraphStyle("CTA", fontName=_FONT, fontSize=11, leading=15, textColor=BLACK, spaceAfter=2*mm))
    return ss


TABLE_STYLE = TableStyle([
    ("FONTNAME", (0,0), (-1,0), _FONT_BOLD), ("FONTNAME", (0,1), (-1,-1), _FONT),
    ("FONTSIZE", (0,0), (-1,-1), 10), ("LEADING", (0,0), (-1,-1), 14),
    ("BACKGROUND", (0,0), (-1,0), LBGR), ("TEXTCOLOR", (0,0), (-1,-1), BLACK),
    ("GRID", (0,0), (-1,-1), 0.5, LINE), ("VALIGN", (0,0), (-1,-1), "MIDDLE"),
    ("TOPPADDING", (0,0), (-1,-1), 3), ("BOTTOMPADDING", (0,0), (-1,-1), 3),
    ("LEFTPADDING", (0,0), (-1,-1), 5),
])

MAX_RETRIES = 3
RETRY_BACKOFF = 2
DELAY_BETWEEN_CALLS = 1.5


def _safe_int(value, default=0) -> int:
    if value is None:
        return default
    try:
        return int(value)
    except (ValueError, TypeError):
        return default


def _safe_float(value, default=0.0) -> float:
    if value is None:
        return default
    try:
        return float(value)
    except (ValueError, TypeError):
        return default


def _build_prompt(lead: dict) -> str:
    niche = NICHE_STATS.get(lead.get("category", ""), {
        "avg_check": 3000, "mobile_search": 75,
        "conversion_low": 0.06, "conversion_high": 0.10,
    })

    competitors_raw = lead.get("competitors_in_search", "[]")
    if isinstance(competitors_raw, str):
        try:
            competitors_list = json.loads(competitors_raw)
        except json.JSONDecodeError:
            competitors_list = []
    else:
        competitors_list = competitors_raw or []

    monthly_searches = _safe_int(lead.get("monthly_searches"))

    data_block = textwrap.dedent(f"""\
        Компания: {lead.get('name', 'N/A')}
        Город: {lead.get('city', 'N/A')}
        Категория: {lead.get('category', 'N/A')}
        Видим в поиске: {'да, позиция ' + str(lead.get('search_position')) if lead.get('search_visible') else 'нет, не найдена в первых 10 результатах'}
        Запросов в месяц по нише: ~{monthly_searches} (оценка на основе данных Яндекс Вордстат)
        Конкурентов в поисковой выдаче: {_safe_int(lead.get('competitors_total'))}
        Из них с собственным сайтом: {_safe_int(lead.get('competitors_with_site'))}
        Конкуренты в выдаче: {', '.join(competitors_list[:5]) if competitors_list else 'данные не получены'}
        Мобильный поиск в нише: {niche['mobile_search']}%
    """)

    prompt = textwrap.dedent(f"""\
        Ты — аналитик digital-маркетинга. Напиши краткий анализ цифрового
        присутствия компании на русском языке.

        КРИТИЧЕСКИЕ ПРАВИЛА:
        - Пиши от лица "мы" (команда аналитиков).
        - Тон: спокойный, экспертный, нейтральный. Как консультант, НЕ как продавец.
        - НЕ начинай с приветствия. Сразу факты.
        - ОБРАЩАЙСЯ на "вы" к владельцу бизнеса.
        - СТРОГО 3 предложения на раздел. Каждое — проверяемый факт.
        - ЗАПРЕЩЕНО: восклицательные знаки, обещания выручки/дохода,
          слова "огромный", "колоссальный", "потенциал", "упущенная выгода",
          "вы теряете", давление, манипуляции, фразы типа "важно понимать".
        - НЕ считай деньги за клиента. НЕ пиши суммы выручки.
        - НЕ упоминай рейтинги и отзывы.

        Формат — 3 раздела:

        1. Видимость в поиске (3 предложения)
        Конкретика: нашли ли компанию в поиске, на какой позиции.
        {niche['mobile_search']}% запросов в нише идут с мобильных.
        2ГИС — каталог, а не замена поисковой выдаче.

        2. Конкурентная среда (3 предложения)
        Назови конкретных конкурентов из выдачи если есть.
        Сколько конкурентов уже с сайтом. Без оценочных суждений.

        3. Спрос в нише (3 предложения)
        Примерный объём запросов. Оговорись что это оценка, не точные данные.
        Часть этого трафика уходит к компаниям с сайтами.

        НЕ ПИШИ блок "что мы сделаем" — он формируется отдельно.

        Объём: 800-1200 символов. Коротко, сухо, по делу.

        === ДАННЫЕ ===
        {data_block}
    """)
    return prompt


def generate_audit_text(api_key, model: str, lead: dict) -> str:
    prompt = _build_prompt(lead)
    keys = api_key if isinstance(api_key, list) else [api_key]

    for i, key in enumerate(keys):
        for attempt in range(MAX_RETRIES):
            try:
                resp = requests.post(
                    f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent",
                    json={"contents": [{"parts": [{"text": prompt}]}]},
                    headers={
                        "Content-Type": "application/json",
                        "x-goog-api-key": key,
                    },
                    timeout=90,
                )
                resp.raise_for_status()
                data = resp.json()
                return data["candidates"][0]["content"]["parts"][0]["text"]
            except requests.exceptions.HTTPError as e:
                status = e.response.status_code if e.response is not None else 0
                if status == 429 and i < len(keys) - 1:
                    logger.warning("Key %d hit 429, switching to key %d", i + 1, i + 2)
                    break
                if status >= 500 and attempt < MAX_RETRIES - 1:
                    wait = RETRY_BACKOFF ** attempt
                    logger.warning("Gemini %d, retry in %ds", status, wait)
                    time.sleep(wait)
                    continue
                raise
            except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                if attempt < MAX_RETRIES - 1:
                    wait = RETRY_BACKOFF ** attempt
                    logger.warning("Connection error, retry in %ds: %s", wait, type(e).__name__)
                    time.sleep(wait)
                    continue
                raise
        else:
            continue
        continue

    raise RuntimeError("All API keys exhausted")


def _sanitize(text: str) -> str:
    text = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    text = re.sub(r"\*\*(.+?)\*\*", r"<b>\1</b>", text)
    text = re.sub(r"`(.+?)`", r"<b>\1</b>", text)
    return text


def _add_text(story, text, style):
    for para in text.split("\n"):
        p = para.strip()
        if not p:
            story.append(Spacer(1, 1.5 * mm))
        else:
            story.append(Paragraph(_sanitize(p), style))


def _split_sections(text: str) -> dict:
    sections = {}
    current = "intro"
    sections[current] = []
    for line in text.split("\n"):
        stripped = line.strip()
        if not stripped:
            sections.setdefault(current, []).append("")
            continue
        clean = re.sub(r"^#{1,4}\s*", "", stripped)
        lower = clean.lower()
        if re.match(r"^\**1[\.\)]", lower) and ("видимость" in lower or "поиск" in lower or "интернет" in lower or "онлайн" in lower):
            current = "vis"; continue
        elif re.match(r"^\**2[\.\)]", lower) and ("конкурент" in lower or "среда" in lower or "рынок" in lower):
            current = "comp"; continue
        elif re.match(r"^\**3[\.\)]", lower) and ("спрос" in lower or "поток" in lower or "клиент" in lower or "запрос" in lower):
            current = "demand"; continue
        sections.setdefault(current, []).append(clean)
    return {k: "\n".join(v).strip() for k, v in sections.items()}


# ---------------------------------------------------------------------------
# PDF generation
# ---------------------------------------------------------------------------

def generate_pdf(lead: dict, audit_text: str, channel: str = "telegram") -> str:
    os.makedirs(PDF_DIR, exist_ok=True)

    safe_name = re.sub(r"[^\w\s-]", "", lead.get("name", "audit")).strip().replace(" ", "_")
    filename = f"Анализ_{safe_name[:40]}.pdf"
    pdf_path = os.path.join(PDF_DIR, filename)

    ss = _make_styles()
    doc = SimpleDocTemplate(
        pdf_path, pagesize=A4,
        topMargin=18*mm, bottomMargin=18*mm, leftMargin=20*mm, rightMargin=20*mm,
    )

    niche = NICHE_STATS.get(lead.get("category", ""), {
        "avg_check": 3000, "mobile_search": 75,
        "conversion_low": 0.06, "conversion_high": 0.10,
    })
    category = lead.get("category", "")
    city = lead.get("city", "")
    name = lead.get("name", "")
    secs = _split_sections(audit_text)
    s = []

    # ===================== PAGE 1 — COVER (professional, minimal) =====================

    s.append(Spacer(1, 50*mm))
    s.append(HRFlowable(width="40%", thickness=1.5, color=ACCENT))
    s.append(Spacer(1, 8*mm))

    s.append(Paragraph(
        "Анализ цифрового присутствия",
        ParagraphStyle("CV1", fontName=_FONT_BOLD, fontSize=22, leading=28,
                       alignment=TA_CENTER, textColor=ACCENT, spaceAfter=4*mm)
    ))
    s.append(Paragraph(
        _sanitize(name),
        ParagraphStyle("CV2", fontName=_FONT_BOLD, fontSize=18, leading=24,
                       alignment=TA_CENTER, textColor=BLACK, spaceAfter=6*mm)
    ))
    s.append(Paragraph(
        f"{category} \u00b7 {city}",
        ParagraphStyle("CV3", fontName=_FONT, fontSize=13, leading=18,
                       alignment=TA_CENTER, textColor=GRAY, spaceAfter=3*mm)
    ))
    s.append(Paragraph(
        datetime.now().strftime("%B %Y").capitalize(),
        ParagraphStyle("CV4", fontName=_FONT, fontSize=12, leading=16,
                       alignment=TA_CENTER, textColor=GRAY, spaceAfter=20*mm)
    ))

    s.append(HRFlowable(width="40%", thickness=0.5, color=LINE))
    s.append(Spacer(1, 10*mm))

    s.append(Paragraph(
        "Подготовлено командой STUN Agency",
        ParagraphStyle("CV5", fontName=_FONT, fontSize=11, leading=14,
                       alignment=TA_CENTER, textColor=GRAY, spaceAfter=2*mm)
    ))
    s.append(Paragraph(
        "stun-agency.ru \u00b7 t.me/stun_agency",
        ParagraphStyle("CV6", fontName=_FONT, fontSize=10, leading=13,
                       alignment=TA_CENTER, textColor=LGRAY)
    ))

    # ===================== PAGE 2 — ANALYSIS =====================

    s.append(Spacer(1, 15*mm))
    s.append(HRFlowable(width="100%", thickness=0.6, color=LINE))
    s.append(Spacer(1, 4*mm))

    # Section 1 — Visibility
    s.append(Paragraph("1. Видимость в поиске", ss["HH"]))
    if secs.get("vis"):
        _add_text(s, secs["vis"], ss["BB"])

    # Section 2 — Competitors
    s.append(Paragraph("2. Конкурентная среда", ss["HH"]))
    if secs.get("comp"):
        _add_text(s, secs["comp"], ss["BB"])

    total = _safe_int(lead.get("competitors_total"), 1) or 1
    with_site = _safe_int(lead.get("competitors_with_site"))
    pct = round(with_site / total * 100) if total else 0
    t2 = Table([
        ["Показатель", "Значение", "Источник"],
        ["Компаний в категории", str(total), "2ГИС"],
        ["Из них с сайтом", f"{with_site} ({pct}%)", "Поисковая выдача"],
        ["Без сайта (вкл. вас)", str(max(total - with_site, 0)), ""],
    ], colWidths=[65*mm, 50*mm, 50*mm])
    t2.setStyle(TABLE_STYLE)
    s.append(KeepTogether([Spacer(1, 2*mm), t2, Spacer(1, 3*mm)]))

    # Section 3 — Demand
    s.append(Paragraph("3. Спрос в нише", ss["HH"]))
    if secs.get("demand"):
        _add_text(s, secs["demand"], ss["BB"])

    monthly_searches = _safe_int(lead.get("monthly_searches"))

    t3 = Table([
        ["Показатель", "Значение", "Источник"],
        ["Запросов в месяц", f"~{monthly_searches}", "Оценка (Яндекс Вордстат)"],
        ["Мобильный поиск", f"{niche['mobile_search']}%", "Среднее по нише"],
    ], colWidths=[65*mm, 50*mm, 50*mm])

    t3_style = TableStyle([
        ("FONTNAME", (0,0), (-1,0), _FONT_BOLD), ("FONTNAME", (0,1), (-1,-1), _FONT),
        ("FONTSIZE", (0,0), (-1,-1), 10), ("LEADING", (0,0), (-1,-1), 14),
        ("BACKGROUND", (0,0), (-1,0), LBGR), ("TEXTCOLOR", (0,0), (-1,-1), BLACK),
        ("GRID", (0,0), (-1,-1), 0.5, LINE),
        ("VALIGN", (0,0), (-1,-1), "MIDDLE"),
        ("TOPPADDING", (0,0), (-1,-1), 3), ("BOTTOMPADDING", (0,0), (-1,-1), 3),
        ("LEFTPADDING", (0,0), (-1,-1), 5),
        ("TEXTCOLOR", (2, 1), (2, -1), GRAY),
        ("FONTSIZE", (2, 1), (2, -1), 8),
    ])
    t3.setStyle(t3_style)
    s.append(KeepTogether([Spacer(1, 2*mm), t3, Spacer(1, 3*mm)]))

    # Seasonality chart
    s.append(Paragraph("Сезонность спроса", ss["HH"]))
    s.append(Paragraph(
        f"Динамика запросов «{category}» по месяцам, % от пикового значения (источник: Яндекс Вордстат):",
        ss["SM"]
    ))
    chart = _make_season_chart(category)
    s.append(KeepTogether([Spacer(1, 2*mm), chart, Spacer(1, 5*mm)]))

    # ===================== PAGE 3 — CTA (soft, personal) =====================

    s.append(HRFlowable(width="100%", thickness=0.6, color=LINE))
    s.append(Spacer(1, 5*mm))

    s.append(Paragraph("Что дальше", ss["HH"]))

    s.append(Paragraph(
        f"Если вам интересно обсудить, как сайт мог бы работать "
        f"для «{_sanitize(name)}», — напишите нам, ответим лично.",
        ss["CTA"]
    ))
    s.append(Spacer(1, 2*mm))
    s.append(Paragraph(
        "Типовое решение для вашей ниши включает: адаптивный сайт, "
        "кнопку звонка, форму заявки, страницу услуг с ценами, карту проезда. "
        "Детали и стоимость обсуждаем индивидуально.",
        ss["CTA"]
    ))

    s.append(Spacer(1, 6*mm))

    # Contact block
    contact_data = [
        ["", ""],
        ["Telegram:", "@stunagncy"],
        ["Канал:", "t.me/stun_agency"],
    ]
    t_contact = Table(contact_data, colWidths=[30*mm, 80*mm])
    t_contact.setStyle(TableStyle([
        ("FONTNAME", (0,0), (-1,-1), _FONT),
        ("FONTSIZE", (0,0), (-1,-1), 11),
        ("TEXTCOLOR", (0,0), (0,-1), GRAY),
        ("TEXTCOLOR", (1,0), (1,-1), BLACK),
        ("LEFTPADDING", (0,0), (-1,-1), 0),
        ("BOTTOMPADDING", (0,0), (-1,-1), 2),
        ("TOPPADDING", (0,0), (-1,-1), 2),
    ]))
    s.append(t_contact)

    s.append(Spacer(1, 6*mm))
    s.append(Paragraph(
        "P.S. Это не массовая рассылка — мы вручную проанализировали "
        "данные по вашей компании и подготовили этот отчёт.",
        ParagraphStyle("PS", fontName=_FONT, fontSize=10, leading=13,
                       textColor=GRAY, fontStyle="italic")
    ))

    # Disclaimer + footer
    s.append(Spacer(1, 8*mm))
    s.append(HRFlowable(width="100%", thickness=0.3, color=LINE))
    s.append(Spacer(1, 2*mm))
    s.append(Paragraph(
        "Оценки основаны на открытых данных (2ГИС, поисковая выдача, Яндекс Вордстат) "
        "и могут отличаться от фактических показателей.",
        ss["FF"]
    ))
    s.append(Paragraph(
        f"STUN Agency \u00b7 {datetime.now().strftime('%d.%m.%Y')}",
        ss["FF"]
    ))

    doc.build(s)
    logger.info("PDF создан: %s", pdf_path)
    return pdf_path


SEASON_DATA = {
    "Шиномонтаж":      [40, 30, 90,100, 60, 30, 25, 30, 50,100, 90, 35],
    "Клининг":          [90, 60, 70, 80, 70, 60, 50, 55, 65, 70, 80,100],
    "Автосервис":       [70, 65, 80, 90, 85, 75, 70, 70, 80, 90, 85, 60],
    "Ремонт телефонов": [80, 70, 75, 70, 65, 60, 55, 60, 80, 75, 70,100],
    "Массаж":           [70, 65, 75, 70, 65, 60, 55, 60, 70, 75, 80, 85],
}
MONTHS = ["Янв","Фев","Мар","Апр","Май","Июн","Июл","Авг","Сен","Окт","Ноя","Дек"]


def _make_season_chart(category: str) -> Drawing:
    data = SEASON_DATA.get(category, [70]*12)
    w, h = 480, 120
    d = Drawing(w, h)
    bar_w = 32
    gap = 8
    max_val = max(data)
    for i, val in enumerate(data):
        x = i * (bar_w + gap) + 10
        bar_h = (val / max_val) * 80
        color = GREEN if val >= 80 else (HexColor("#f5a623") if val >= 50 else HexColor("#cc6666"))
        d.add(Rect(x, 20, bar_w, bar_h, fillColor=color, strokeColor=None))
        d.add(String(x + bar_w/2, 5, MONTHS[i], fontSize=7, fontName=_FONT, textAnchor="middle", fillColor=GRAY))
        d.add(String(x + bar_w/2, 22 + bar_h, f"{val}%", fontSize=6, fontName=_FONT, textAnchor="middle", fillColor=BLACK))
    return d


def _load_audits() -> list[dict]:
    if os.path.exists(AUDITS_JSON):
        with open(AUDITS_JSON, "r", encoding="utf-8") as f:
            return json.load(f)
    return []


def _save_audits(audits: list[dict]):
    os.makedirs(os.path.dirname(AUDITS_JSON), exist_ok=True)
    tmp_fd, tmp_path = tempfile.mkstemp(
        dir=os.path.dirname(AUDITS_JSON), suffix=".tmp"
    )
    try:
        with os.fdopen(tmp_fd, "w", encoding="utf-8") as f:
            json.dump(audits, f, ensure_ascii=False, indent=2)
        os.replace(tmp_path, AUDITS_JSON)
    except Exception:
        if os.path.exists(tmp_path):
            os.unlink(tmp_path)
        raise


def _append_log(lead: dict, pdf_path: str):
    os.makedirs(os.path.dirname(AUDIT_LOG), exist_ok=True)
    with open(AUDIT_LOG, "a", encoding="utf-8") as f:
        f.write(
            f"- [{datetime.now().strftime('%Y-%m-%d %H:%M')}] "
            f"**{lead.get('name')}** ({lead.get('city')}, {lead.get('category')}) → "
            f"`{os.path.basename(pdf_path)}`\n"
        )


def run(config: dict):
    init_db()

    api_keys = config.get("gemini_api_keys", [])
    api_key = api_keys if api_keys else config.get("gemini_api_key", "")
    if not api_key or (isinstance(api_key, str) and api_key.startswith("YOUR_")):
        logger.error("Gemini API key не задан в config.json")
        return

    model = config.get("gemini_model", "gemini-2.5-flash")

    leads = get_leads_for_audit()
    if not leads:
        logger.info("Нет лидов для аудита")
        return

    logger.info("Найдено %d лидов для аудита", len(leads))

    audits = _load_audits()
    processed = 0

    for lead in leads:
        lead_id = lead["id"]
        lead_name = lead.get("name", "???")
        logger.info("[%d/%d] Генерация аудита: %s", processed + 1, len(leads), lead_name)

        try:
            audit_text = generate_audit_text(api_key, model, lead)
            logger.info("Текст: %d символов", len(audit_text))

            pdf_path = generate_pdf(lead, audit_text)

            openers = [
                f"Добрый день. Мы — команда STUN Agency, занимаемся созданием сайтов для бизнеса.",
                f"Добрый день. Мы — веб-студия STUN Agency, делаем сайты для малого бизнеса.",
                f"Здравствуйте. Мы — команда STUN Agency, помогаем бизнесу выходить в онлайн.",
            ]
            bodies = [
                f"Мы подготовили краткий анализ цифрового присутствия вашей компании «{lead_name}» — в приложении.",
                f"Мы проанализировали онлайн-присутствие «{lead_name}» и подготовили отчёт — во вложении.",
                f"Подготовили для вас анализ присутствия «{lead_name}» в поисковых системах — файл в приложении.",
            ]
            closers = [
                "Если не хотите открывать файл — напишите, продублируем текстом.",
                "Если неудобно открывать PDF — скажите, отправим текстом.",
            ]
            message_text = (
                f"{random.choice(openers)}\n\n"
                f"{random.choice(bodies)}\n\n"
                f"{random.choice(closers)}"
            )

            audit_record = {
                "lead_id": lead_id,
                "name": lead_name,
                "city": lead.get("city", ""),
                "category": lead.get("category", ""),
                "audit_text": audit_text,
                "audit_pdf_path": pdf_path,
                "message_text": message_text,
                "generated_at": datetime.now().isoformat(),
            }
            audits.append(audit_record)
            _save_audits(audits)

            mark_audit_generated(lead_id)
            _append_log(lead, pdf_path)

            processed += 1
            logger.info("Аудит сохранён: «%s»", lead_name)

        except requests.exceptions.HTTPError as exc:
            status = exc.response.status_code if exc.response is not None else "unknown"
            logger.error("Gemini API error «%s»: HTTP %s", lead_name, status)
            continue
        except Exception as exc:
            logger.error("Ошибка «%s»: %s", lead_name, type(exc).__name__, exc_info=True)
            continue

        time.sleep(DELAY_BETWEEN_CALLS)

    logger.info("Готово: %d/%d аудитов", processed, len(leads))


if __name__ == "__main__":
    config_path = os.path.join(PROJECT_ROOT, "config.json")
    with open(config_path, "r", encoding="utf-8") as f:
        cfg = json.load(f)
    run(cfg)
